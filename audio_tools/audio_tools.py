...
from audio_tools.fbtools import fft2melmx
...
def get_envelope(audio, audio_fs, new_fs, cof=25, bef_aft=[0, 0], pad_next_pow2=False):
    ...
    if pad_next_pow2:
        ...
        orig_len = len(audio)
        sound_pad = np.hstack(
            (audio, np.zeros((2 ** int(np.ceil(np.log2(len(audio)))) - len(audio),)))
        )
        audio = sound_pad
    ...
    nyq = audio_fs / 2.0  # Nyquist frequency
    ...
    envelope_long = np.atleast_2d(
        filtfilt(b, a, env_hilb, axis=0)
    )  # filtfilt makes it non-causal (fwd/backward)

    envelope = resample(
        envelope_long.T, int(np.floor(envelope_long.shape[1] / (audio_fs / new_fs)))
    )
    if pad_next_pow2:
    ...
    if bef_aft[0] < 0:
        ...
        envelope = np.vstack(
            (np.zeros((int(np.abs(bef_aft[0]) * new_fs), 1)), envelope)
        )
    if bef_aft[1] > 0:
...
def get_cse_onset(
    ...
    if pos_deriv is False:
        auddiff = np.sum(
            np.diff(
                np.hstack((np.atleast_2d(specgram_thresh[:, 0]).T, specgram_thresh))
            )
            ** 2,
            axis=0,
        )
    ...
    for wi, w in enumerate(win_segments):
        box = np.hstack(
            (np.atleast_2d(boxcar(w)), -np.ones((1, int(0.15 * new_fs))))
        ).ravel()
...
def get_mel_spectrogram(
    w, fs, wintime=0.025, steptime=0.010, nfilts=80, minfreq=0, maxfreq=None
):
...
def audspec(
    ...
    if sumpower:
        ...
        aspectrum = np.dot(wts, np.sqrt(pspectrum)) ** 2.0
...
if __name__ == "main":
    ...
    print(stim_fs)
    envelope = get_envelope(w, stim_fs, eeg_fs, pad_next_pow2=True)
